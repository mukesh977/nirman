            ...
            <div class="idr-content idrc-orders-details">
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="odb-left">
                                <h6>Order Detial</h6>
                                <ul>
                                    <li>Order Id: {{ $invoice->order->id }}</li>
                                    <li>Customer Name: {{ $invoice->order->name }}</li>
                                    <li>Status: <span class="badge badge-info"
                                            style="font-size: 13px;">{{ $invoice->order->status }}</span>
                                    </li>
                                </ul>
                                <h6>Shipping Address</h6>
                                <ul>
                                    <li>{{ $invoice->order->city }}</li>
                                    <li>{{ $invoice->order->phone }}</li>
                        ...
                        {{-- <div class="col-lg-6 ">
                            <div>
                            <form action="{{ route('admin.invoice.destroy',$invoice->id) }}" class="form-inline" method="POST">
                                    @csrf
                                    @method('delete')
                ...
                <div class="orders-details-middle">
                    <table>
                        ...
                        <tbody>
                            @foreach ( json_decode($invoice->order->description) as $item)
